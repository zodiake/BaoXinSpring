    ...
    <div class="detail-table buyerOrderList">
        ...
        <div class="text-right" id="myOrders">
            <c:if test="${grid.totalPages>1}">
                <ul class="pagination">
                    <li><a data-url="${pageContext.request.contextPath}/orderCenter/buyerOrderList?page=${grid.currentPage-1<1?1:grid.currentPage-1}">上一页</a></li>
                    ...
                    <c:forEach varStatus="status" begin="${grid.currentPage-3<1?1:grid.currentPage-3}" end="${grid.currentPage+3>grid.totalPages?grid.totalPages:grid.currentPage+3}">
                        <li <c:if test='${grid.currentPage == status.current}'> class="active"</c:if>>
                            <a data-url="${pageContext.request.contextPath}/orderCenter/buyerOrderList?page=${status.current}">${status.current}</a>
                        </li>
                    </c:forEach>

                    <li><a data-url="${pageContext.request.contextPath}/orderCenter/buyerOrderList?page=${grid.currentPage+1>grid.totalPages?grid.totalPages:grid.currentPage+1}">下一页</a></li>
                </ul>
            </c:if>
        </div>
    </div>
