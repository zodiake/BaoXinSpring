            ...
            <ul>
                <c:forEach var="item" items="${grid.ecList}">
                    <li>
                        ...
                        <div class="bargain">
                            成交${item.bidCount}笔<div class="bargain-fun"><%--<a href="#" title="" class="del-icon deleteFavouriteItem" data-id="${item.id}"></a>--%><a href="/shopCenter/${item.createdBy.id}/items" title="供应商主页" class="home-icon"></a><a  title="加入购物车" class="shopping-car-icon addCart" data-id="${item.id}"></a><a title="加入调样册" class="album-icon addBook" data-id="${item.id}"></a></div>
                        </div>
                    </li>
                </c:forEach>
            </ul>
