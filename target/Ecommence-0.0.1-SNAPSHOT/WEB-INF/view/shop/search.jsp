                ...
                <tr>
                    ...
                    <td>
                        <ul class="list-inline season">
                            <form:checkboxes path="type" items="${types}"  element="li" data-label="主营业务"></form:checkboxes>
                        </ul>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <ul class="list-inline weight">
                            ...
                            <li>
                                <form:input path="rangeMinMoney" type="text" class="w-40 input-small"/>
                                ~
                                <form:input path="rangeMaxMoney" type="text" class="w-40 input-small"/>
                        ...
                        <tr>
                            <td width="200">
                                <a href="${pageContext.request.contextPath}/shopCenter/${list._id}/items">
                                    <c:choose>
                                        <c:when test="${not empty list.cover}"><img class="thumb" src="${pageContext.request.contextPath}${list.cover}"></c:when>
                            ...
                            <td>
                                ...
                                <p class="gray">
                                    <c:if test="${not empty list.type}"><span>供应商品类型：${list.type}</span>|</c:if>
                ...
                <c:if test="${pageCount>1}">
                    <div class="text-right">
                        <ul class="pagination page">
                            <li><a data-page="${currentPage-1<1?1:currentPage-1}">上一页</a></li>
                            <c:forEach varStatus="status" begin="${currentPage-2<=1?1:currentPage-2}"
                                       end="${currentPage+4>=pageCount?pageCount:page+3}">
                                <li <c:if test='${currentPage == status.current}'>class="active"</c:if>>
                                    <a data-page="${status.current}">${status.current}</a>
                                </li>
                            </c:forEach>
                            <li><a data-page="${currentPage+1>pageCount?pageCount:currentPage+1}">下一页</a></li>
                        </ul>
                    </div>
                </c:if>
            </c:when>
            ...
            <c:forEach items="${shops}" var="shop">
                <div class="col-1">
                    <a href="${pageContext.request.contextPath}/shopCenter/${shop.id}/items">
                        <c:choose>
                            <c:when test="${not empty shop.cover}"><img class="thumb" src="${pageContext.request.contextPath}${shop.cover}"></c:when>
                            <c:otherwise><img class="thumb" src="${pageContext.request.contextPath}/resources/img/btn-th.png"></c:otherwise>
                        </c:choose>
                    </a>
                    <ul>
                        <li class="text-over">
                            <span><a href="${pageContext.request.contextPath}/shopCenter/${shop.id}/items" title="${shop.name}">${shop.name}</a></span>
                        </li>
                        <li>
                            <label>综合评分：</label>
