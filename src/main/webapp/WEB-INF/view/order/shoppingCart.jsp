...
<div class="container">
    ...
    <div class="order-detail">
        <c:choose>
            <c:when test="${cartLineList == null || cartLineList.size == 0 || cartLineList == '{}'}">
                    <div class="central">
                        ...
                        <c:forEach var="cartLine" items="${cartLineList}">
                            ...
                            <c:forEach items="${cartLine.value}" var="cartItem">
                                ...
                                <tr class="dataTable_${cartItem.itemId}">
                                    <td colspan="2">
                                        <div class="central">
                                            <a href="${pageContext.request.contextPath}${cartItem.itemType}">
                                            ...
                                            <a href="${pageContext.request.contextPath}${cartItem.itemType}">${cartItem.title}</a>
                                        </div>
                                    </td>
                                    <td><span id="${cartItem.itemId}_price">
                                        <c:choose>
                                            <c:when test="${cartItem.price==0}">
                                                面议
                                            </c:when>
                                            <c:otherwise>${cartItem.price}</c:otherwise>
                                        </c:choose>
                                    </span></td>
                                    <td>
                                    ...
                                    <td><span id="${cartItem.itemId}_summary">
                                    <c:choose>
                                        <c:when test="${cartItem.summary==0}">
                                            面议
                                        </c:when>
                                        <c:otherwise>${cartItem.summary}</c:otherwise>
                                    </c:choose>
                                    </span></td>
                                    <td>
                            ...
                            <tr align="center">
                                ...
                                <td align="center">商品金额<br/>
                                    <span class="sumprice_${cartLine.key}">
                                        <c:choose>
                                            <c:when test="${sumprice==0}">
                                                面议
                                            </c:when>
                                            <c:otherwise><b class="orange"><fmt:formatNumber type="currency" pattern="￥.00" value="${sumprice}"></fmt:formatNumber></b></c:otherwise>
                                        </c:choose>
                                     </span>
                                    </td>
                                <td>
                                    <a type="button" class="button button-deep button-big submitCart" href="#" data-key="${cartLine.key}">生成订单</a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                    <tfoot>
...
<form id="cartForm" action="/orderCenter/checkOut" method="post">
