...
<div class="container">
    ...
    <dl class="box order-info">
        ...
        <dd>
            <table class="table table-border table-order">
                ...
                <tbody>
                <c:forEach items="${sampleBookList}" var="item">
                    <tr class="dataTable_${item.id}">
                        <td><a href="${pageContext.request.contextPath}${item.url}">${item.name}</a></td>
    ...
    <div class="cart-submit"><p class="right"><input type="button" value="返回调样册" class="button button-default" onclick="javascript:window.history.back();"/><button type="button" class="button button-deep" onclick="preSubmit()">提交调样</button></p></div>
    <!-- /.Cart Order-Info -->
</div>
<!-- /.Container -->
</form>
