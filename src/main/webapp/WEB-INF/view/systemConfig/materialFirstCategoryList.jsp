...
<div>
    <a  class="button button-deep" href="${pageContext.request.contextPath}/admin/materialFirstCategory/add">添加辅料产品类别</a><br/><br/>
        ...
        <table class="table table-border">
            ...
            <c:forEach var="firstCategory" items="${grid.ecList}">
                <tr class="tr-${firstCategory.id}">
                    ...
                    <td><a href="${pageContext.request.contextPath}/admin/materialSecondCategory/add/${firstCategory.id}">${firstCategory.name}</a></td>
    ...
    <div class="text-right">
        <c:if test="${grid.totalPages>1}">
            <%--<ul class="pagination navigationTo">--%>
                <%--<li><a href="${pageContext.request.contextPath}/admin/materialFirstCategory?page=1">首页</a></li>--%>
                ...
                <%--<c:forEach varStatus="status" begin="${grid.currentPage-3<1?1:grid.currentPage-3}" end="${grid.currentPage+3>grid.totalPages?grid.totalPages:grid.currentPage+3}">--%>
                    <%--<li <c:if test='${grid.currentPage == status.current}'> class="active"</c:if>>--%>
                        <%--<a href="${pageContext.request.contextPath}/admin/materialFirstCategory?page=${status.current}">${status.current}</a>--%>
                    <%--</li>--%>
                <%--</c:forEach>--%>

                <%--<li><a href="${pageContext.request.contextPath}/admin/materialFirstCategory?page=${grid.totalPages}">末页</a></li>--%>
            <%--</ul>--%>
            <sbTag:page total="${grid.totalPages}" current="${grid.currentPage}" href="${pageContext.request.contextPath}/admin/materialFirstCategory?1=1"></sbTag:page>
        </c:if>
    </div>
</div>