...
<div>
    <a  class="button button-deep" href="${pageContext.request.contextPath}/admin/fabricSecondCategory/add">添加产品二级类别</a><br/><br/>
    ...
    <div class="text-right">
        <c:if test="${grid.totalPages>1}">
            <%--<ul class="pagination navigationTo">--%>
                <%--<li><a href="${pageContext.request.contextPath}/admin/fabricSecondCategory?page=1">首页</a></li>--%>
                <%--<c:forEach varStatus="status" begin="${grid.currentPage-3<1?1:grid.currentPage-3}" end="${grid.currentPage+3>grid.totalPages?grid.totalPages:grid.currentPage+3}">--%>
                    <%--<li <c:if test='${grid.currentPage == status.current}'> class="active"</c:if>>--%>
                        <%--<a href="${pageContext.request.contextPath}/admin/fabricSecondCategory?page=${status.current}">${status.current}</a>--%>
                    <%--</li>--%>
                <%--</c:forEach>--%>
                <%--<li><a href="${pageContext.request.contextPath}/admin/fabricSecondCategory?page=${grid.totalPages}">末页</a></li>--%>
            <%--</ul>--%>
            <sbTag:page total="${grid.totalPages}" current="${grid.currentPage}" href="${pageContext.request.contextPath}/admin/fabricSecondCategory?1=1"></sbTag:page>
        </c:if>
    </div>
</div>