...
<div class="main">
    ...
    <div class="attention-commodity">
                    ...
                    <td class="col-red">
                        <c:choose>
                            <c:when test="${favouriteItem.item.price > 0}">
                                ${favouriteItem.item.price}
                            </c:when>
                            <c:otherwise>
                                面议
                            </c:otherwise>
                        </c:choose>
                        (${favouriteItem.item.state})
                    </td>
                    <td>
                        <%--<c:choose>--%>
                            <%--<c:when test="${favouriteItem.item.price > 0}">--%>
        ...
        <c:if test="${grid.totalPages>1}">
            ...
            <sbTag:page total="${grid.totalPages}" current="${grid.currentPage}" href="${pageContext.request.contextPath}/buyerCenter/favouritesList?1=1"></sbTag:page>
        </c:if>
        <!--翻页 end-->
    </div>
    <!--关注的供应商 end-->
