...
<li>
    <div class="fox">
        ...
        <ul class="list-inline">
            <li><a href="javascript:void(0);">染色与整理类</a></li>
    ...
    <div class="side">
        ...
        <div class="data">
							...
							<span>
								...
								<p>设计师<bdo class="orange">${statistics['designerCount']}</bdo>名，品牌<bdo class="orange">${statistics['brandCount']}</bdo>家</p>
                ...
                <div class="grid">
                    <ul>
                        <c:forEach items="${fabricShop}" var="fShop">
                            <li>
                                <a href="${fShop.link}" target="_blank">
                                    <c:choose>
                    ...
                    <c:forEach items="${fabricList}" var="fabric">
                        <li class="col-1">
                            <a href="${pageContext.request.contextPath}/${fabric.url}/${fabric.id}">
                                ...
                                <span>
                                    <c:choose>
                                        <c:when test="${fabric.price > 0.0}">
                                            <bdo class="price orange">${fabric.price}</bdo>/米
                                        </c:when>
                                        <c:otherwise>
                                            <bdo class="orange">面议</bdo>
                                        </c:otherwise>
                                    </c:choose>
                ...
                <div class="grid">
                    <ul>
                        <c:forEach items="${materialShop}" var="mShop">
                            <li>
                                <a href="${mShop.link}" target="_blank">
                                    <c:choose>
                                ...
                                <span>
                                    <c:choose>
                                        <c:when test="${material.price > 0.0}">
                                            <bdo class="price orange">${material.price}</bdo>/米
                                        </c:when>
                                        <c:otherwise>
                                            <bdo class="orange">面议</bdo>
                                        </c:otherwise>
            ...
            <c:forEach items="${designerList}" var="designer">
                <li class="col-1">
                    <c:choose>
                        ...
                        <c:choose>
                            <c:when test="${not empty designer.headUrl}"><img class="thumb" src="${linkMap['designerImg']}${designer.headUrl}&imageSize=200"></c:when>
                            <c:otherwise><img class="thumb" src="${pageContext.request.contextPath}/resources/img/btn-th.png"></c:otherwise>
                        </c:choose>
                        <div>
                            <b>${designer.name}</b>
                ...
                <c:choose>
                    <c:when test="${i.index<1}">
                        <li class="col-2">
                            ...
                            <c:choose>
                                <c:when test="${not empty brand.headUrl}"><img src="${linkMap['designerImg']}${brand.headUrl}" class="thumb" style="cursor: pointer;"></c:when>
                                <c:otherwise><img class="thumb" src="${pageContext.request.contextPath}/resources/img/btn-th.png" style="cursor: pointer"></c:otherwise>
                            </c:choose>
                    ...
                    <c:when test="${i.index>0}">
                        <li class="col-1">
                            ...
                            <c:choose>
                                <c:when test="${not empty brand.headUrl}"><img src="${linkMap['designerImg']}${brand.headUrl}&imageSize=200" class="thumb" style="cursor: pointer"></c:when>
                                <c:otherwise><img class="thumb" src="${pageContext.request.contextPath}/resources/img/btn-th.png" style="cursor: pointer"></c:otherwise>
                            </c:choose>
                        </li>
                    </c:when>
...
<div class="col-3">
    <c:if test="${not empty masterViewList}">
        <div style="height: 182px;">
            <div class="masterView"><i style="margin-right: 5px;padding: 0 2px;border-radius: 4px;background: #999;"></i>专家访谈
                <span style="margin-left:172px;"><a class="more" href="${pageContext.request.contextPath}/masterView" target="_blank">更多</a></span>
            </div>
            <div class="filmDiv">
                <c:forEach var="masterView" items="${masterViewList}" varStatus="i">
                    <c:if test="${i.index<3}">
                        <span>
                            <a href="${pageContext.request.contextPath}/masterView/${masterView.id}" target="_blank">
                                <img class="filmpic${i.index<1?1:2}" src="${pageContext.request.contextPath}${masterView.viewPath}">
                            </a>
                        </span>
                    </c:if>
                </c:forEach>
    ...
    <div class="box">
        <a href="/topic1022" target="_blank">
            <img class="thumb" src="/resources/images/MediaFocus.gif">
        </a>
    </div>
    <!-- /.Tab -->
    <div class="home-message-box sider box">
        <div class="tabs">
            <ul class="tabsRow">
                <li id="tabs1" class="tabs-col"><a href="${pageContext.request.contextPath}/notice" class="active"onMouseMove="switchTab('tabs1','con1');this.blur();return false;" target="_blank">公告</a></li>
                <li id="tabs2" class="tabs-col"><a href="${pageContext.request.contextPath}/info" onMouseMove="switchTab('tabs2','con2');this.blur();return false;" target="_blank" >资讯</a></li>
                <li id="tabs3" class="tabs-col"><a href="#" onMouseMove="switchTab('tabs3','con3');this.blur();return false;">订货会</a></li>
            </ul>
        </div>
        ...
        <div id="con3" class="message-list" style="display:none;">
            <ul>
                <c:forEach var="ordering" items="${orderingList}">
                    <li>
                        <a href="http://www.cloudfashion.org/sccpm/DispatchAction.do?efFormEname=PMOS004&orderShowId=${ordering.id}" target="_blank">${ordering.name}</a>
                    </li>
                </c:forEach>
            </ul>
        </div>
            ...
            <dd>
                ...
                <ul class="row-3">
                    <c:forEach items="${ladiesFabric}" var="ladyFabric">
                        <li class="col-1">
                            <a href="${ladyFabric.link}"><img class="thumb" src="${pageContext.request.contextPath}${ladyFabric.coverPath}?size=100"></a>
                        </li>
                    </c:forEach>
                </ul>
                <b>男装面料</b>
                <ul class="row-3">
                    <c:forEach items="${mensFabric}" var="menFabric">
                        <li class="col-1">
                            <a href="${menFabric.link}"><img class="thumb" src="${pageContext.request.contextPath}${menFabric.coverPath}?size=100"></a>
                        </li>
                    </c:forEach>
                </ul>
            </dd>
