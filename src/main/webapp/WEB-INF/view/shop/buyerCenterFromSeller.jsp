...
<div class="main">
    <dl class="detail-main assessment-tab">
        ...
        <ul class="list-inline">
            <li id="fromSeller" class="active">卖家评我</li>
        ...
        <dd>
            <table class="table table-none">
                ...
                <tr id="fromSellerTab">
                    <th width="20%">
                        <select data-id="${id}" data-url="${pageContext.request.contextPath}/buyerCenter/comments/fromSeller" data-flag="fromBuyer">
                    ...
                    <td colspan="4">
                        <div class="text-right" id="fromSellerPage">
                            <c:if test="${grid.totalPages>1}">
                                <%--<ul class="pagination">
                                    <li><a href="${pageContext.request.contextPath}/buyerCenter/comments/fromSeller?type=${type}&page=${grid.currentPage-1<1?1:grid.currentPage-1}">上一页</a></li>
                                    ...
                                    <c:forEach varStatus="status" begin="${grid.currentPage-3<=0?1:grid.currentPage-3}" end="${grid.currentPage+3>grid.totalPages?grid.totalPages:grid.currentPage+4}">
                                        <li <c:if test='${grid.currentPage == status.current}'> class="active"</c:if>>
                                            <a href="${pageContext.request.contextPath}/buyerCenter/comments/fromSeller?type=${type}&page=${status.current}">${status.current}</a>
                                        </li>
                                    </c:forEach>

                                    <li><a href="${pageContext.request.contextPath}/buyerCenter/comments/fromSeller?type=${type}&page=${grid.currentPage+1>grid.totalPages?grid.totalPages:grid.currentPage+1}">下一页</a></li>
                                </ul>--%>
                                <sbTag:page total="${grid.totalPages}" current="${grid.currentPage}" href="${pageContext.request.contextPath}/buyerCenter/comments/fromSeller?type=${type}"></sbTag:page>
                            </c:if>
                        </div>
                    </td>
                </tr>
