        ...
        <td colspan="4">
            <div class="text-right" id="fromSellerSendPage">
                <c:if test="${grid.totalPages>0}">
                    <%--<ul class="pagination">
                        <li><a data-url="${pageContext.request.contextPath}/sellerCenter/${id}/sellerComments?page=1">上一页</a></li>
                        ...
                        <c:forEach varStatus="status" begin="${grid.currentPage-3<=0?1:grid.currentPage-3}" end="${grid.currentPage+3>grid.totalPages?grid.totalPages:grid.currentPage+4}">
                            <li <c:if test='${grid.currentPage == status.current}'> class="active"</c:if>>
                                <a data-url="${pageContext.request.contextPath}/sellerCenter/${id}/sellerComments?page=${status.current}">${status.current}</a>
                            </li>
                        </c:forEach>

                        <li><a data-url="${pageContext.request.contextPath}/sellerCenter/${id}/sellerComments?page=${grid.totalPages}">下一页</a></li>
                    </ul>--%>
                    <sbTag:page total="${grid.totalPages}" current="${grid.currentPage}" href="${pageContext.request.contextPath}/sellerCenter/${id}/sellerComments?1=1" dataUrl="${pageContext.request.contextPath}/sellerCenter/${id}/sellerComments?1=1"></sbTag:page>
                </c:if>
            </div>
        </td>
    </tr>
